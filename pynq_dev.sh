...
wire_service_panes() {
  ...
  tmux send-keys -t "$SESSION:0.0" "clear; printf '\033[1;38;5;201m%s\033[0m\n' 'PYNQ SERVER'; printf '\033[2m%s\033[0m\n' 'pynq_full/ec2/server/server.py'; printf '\033[36m● %s\033[0m\n\n' 'Connecting to EC2 and starting server...'; ssh $SSH_OPTS -t -i \"$KEY\" \"$EC2\" 'source ~/venv/bin/activate && cd ~/ServerSide_PYNQ_Raycaster/pynq_full/ec2/server && python3 server.py'" Enter
  ...
  tmux send-keys -t "$SESSION:0.2" "clear; printf '\033[1;38;5;201m%s\033[0m\n' 'MONITOR TUNNEL'; printf '\033[2m%s\033[0m\n' 'local :8080 -> EC2 :8080'; printf '\033[36m● %s\033[0m\n\n' 'Opening SSH tunnel and waiting for monitor...'; fuser -k 8080/tcp 2>/dev/null || true; ssh $SSH_OPTS -o ExitOnForwardFailure=yes -t -i \"$KEY\" -L 0.0.0.0:8080:localhost:8080 \"$EC2\" 'source ~/venv/bin/activate && cd ~/ServerSide_PYNQ_Raycaster/pynq_full/ec2/monitor && nohup python3 monitor.py > /tmp/monitor.log 2>&1 & until nc -z localhost 8080 2>/dev/null; do sleep 0.2; done && echo [monitor] port 8080 ready && tail -f /tmp/monitor.log'" Enter
